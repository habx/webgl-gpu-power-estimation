(function (global, factory) {
  ...
  function detectAppleDevice(gpu) {
    ...
    var type;
    var userAgent = window.navigator.userAgent.toLowerCase(); // WIP
    ...
    devices.forEach(function (device) {
      if (device.width !== width || device.height !== height || device.pixelRatio !== pixelRatio || device.type && device.type !== type) {
        return;
      }
      ...
      if (appleDevices && appleDevices.length) {
        ...
        gpus.forEach(function (gpu) {
          ...
          if (gpu.name) {
            ...
            (_result$names = result.names).push.apply(_result$names, _toConsumableArray(gpu.names));
          }
